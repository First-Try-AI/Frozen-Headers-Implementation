...
const { extractArticleText, validateArticleTextInput } = require('./articleProcessor');
...
app.post('/process-input', async (req, res) => {
  try {
    ...
    // Use the shared audio workflow function
    const result = await processAudioWorkflow({ userText, originalParams, sessionId, customVoices, thresholds });
...
// Reusable function: Process audio workflow (extracted from /process-input for reuse)
async function processAudioWorkflow({ userText, originalParams, sessionId, customVoices, thresholds }) {
  console.log(`🎯 [AUDIO-WORKFLOW] Starting processing for session: ${sessionId}`);
  console.log(`📝 [AUDIO-WORKFLOW] Input length: ${userText.length} characters`);
    ...
    console.log(`📦 [AUDIO-WORKFLOW] Created ${chunks.length} chunks using ${chunkResult.markerUsed} marker`);
    console.log(`📦 [AUDIO-WORKFLOW] Primary chunks: ${chunkResult.primaryChunks}, Secondary chunks: ${chunkResult.secondaryChunks}`);
    } else {
    console.log(`📦 [AUDIO-WORKFLOW] Created ${chunks.length} chunks (traditional chunking)`);
    }
    ...
    if (originalParams && originalParams.speakerMode === 'oneVoice') {
      ...
      console.log(`[AUDIO-WORKFLOW] 'oneVoice' mode active. Using single voice ID for all chunks: ${singleVoiceForSession}`);
    }
  ...
  console.log(`🎭 [AUDIO-WORKFLOW] Structured ${structuredChunks.length} chunks`);

    // Step 3: Route based on actual chunk count
    if (structuredChunks.length === 1) {
    console.log(`🔄 [AUDIO-WORKFLOW] Routing to single chunk processing`);
      const result = await processSingleChunk(structuredChunks[0], sessionId, thresholds, userText);
    return result;
    } else {
    console.log(`🔄 [AUDIO-WORKFLOW] Routing to multi-chunk processing`);
      const result = await processAllChunks(structuredChunks, sessionId, thresholds, userText);
    return result;
  }
}

// Article text extraction endpoint - fetches article from URL, extracts clean text then processes as audio
app.post('/process-article-text', async (req, res) => {
  try {
    const { url, sessionId, originalParams, customVoices, thresholds } = req.body;

    // Validate required fields
    if (!url || typeof url !== 'string') {
      return res.status(400).json({
        error: 'Invalid url: url is required and must be a string'
      });
    }
    ...
    console.log(`📄 [ARTICLE-TEXT] Starting processing for session: ${sessionId}`);
    console.log(`📄 [ARTICLE-TEXT] URL: ${url}`);

    // Step 1: Validate URL input
    validateArticleTextInput(url);

    // Step 2: Extract clean article text
    console.log(`📄 [ARTICLE-TEXT] Extracting clean article text...`);
    const extractionResult = await extractArticleText(url);
    const { cleanedText, articleMetadata } = extractionResult;

    console.log(`📄 [ARTICLE-TEXT] Extracted text length: ${cleanedText.length} characters`);
    console.log(`📄 [ARTICLE-TEXT] Article: "${articleMetadata.title}" from ${articleMetadata.source}`);

    // Step 3: Process the cleaned text through the existing audio workflow
    console.log(`📄 [ARTICLE-TEXT] Processing cleaned text through audio synthesis...`);
    ...
    // Call the existing /process-input logic internally
    const audioResult = await processAudioWorkflow(audioRequest);

    // Add article metadata to the response
    audioResult.articleMetadata = articleMetadata;

    return res.json(audioResult);

  } catch (error) {
    console.error('❌ [ARTICLE-TEXT] Error:', error);
    res.status(500).json({
      error: 'Internal server error during article text processing',
      details: error.message
    });
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(`📄 [SERVER] Article text endpoint: http://localhost:${PORT}/process-article-text`);
});
