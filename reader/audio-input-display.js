...
class AudioSystemInputDataDisplay {
    ...
    _displayFullChunkInternal(chunkIndex) {
        ...
        // Add the translucent refresh button to text-title
        this.addRefreshButtonToTextTitle(textTitle, chunkIndex);
    ...
    _updateTextDisplayForPageInternal(pageIndex) {
        ...
        // Clear all active word highlighting before showing new page
        const allWords = document.querySelectorAll('.word.active');
        ...
        // DEBUG: Detect word overflow after rendering
        setTimeout(() => {
            const activeWords = document.querySelectorAll('.word.active');
            activeWords.forEach(word => {
                const rect = word.getBoundingClientRect();
                const lineRect = word.closest('.text-line')?.getBoundingClientRect();
                const contentRect = word.closest('.text-content')?.getBoundingClientRect();

                if (lineRect && (rect.right > lineRect.right || rect.left < lineRect.left)) {
                    // Word overflows line boundaries
                }

                if (contentRect && (rect.right > contentRect.right || rect.left < contentRect.left)) {
                    // Word overflows content boundaries
                }
            });
        }, 100);
        });
    ...
    addRefreshButtonToTextTitle(textTitle, chunkIndex) {
        // Create refresh button with translucent styling
        ...
        refreshButton.style.border = '1px solid rgba(255, 255, 255, 0.1)';
        refreshButton.style.color = 'rgba(255, 255, 255, 0.6)';
        ...
        refreshButton.addEventListener('mouseenter', () => {
            ...
            refreshButton.style.color = 'rgba(255, 255, 255, 0.8)';
            refreshButton.style.opacity = '1';
            refreshButton.style.transform = 'rotate(180deg) scale(1.05)';
        });
        ...
        textTitle.appendChild(refreshButton);
    }
