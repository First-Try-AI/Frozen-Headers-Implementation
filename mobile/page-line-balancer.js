...
class PageLineBalancer {
    ...
    initializeMeasurement(container, fontFamily, fontSize) {
        ...
        // Measure at the 1.13x "active" size to pre-allocate space
        ...
        // Measure actual .text-content element
        const textContent = container.querySelector('.text-content');
        if (textContent) {
            this.containerWidth = textContent.getBoundingClientRect().width;

            // Set workingWidth to 100% of the textContent's width.
            // The font measurement is now pre-scaled, so no safety margin is needed.
            this.workingWidth = this.containerWidth;
        } else {
            // Fallback to CSS-based calculation
            this.workingWidth = Math.floor(this.containerWidth * 0.9);
        }

        // DEBUG: Log container measurements
        console.log(`📐 [PAGE-LINE-BALANCER] Container: ${this.containerWidth}px, Working width: ${this.workingWidth}px (measures at 1.13x scale)`);
    ...
    distributeWordsIntoLines(words) {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            // DEBUG: Log width comparison for each word
            console.log(`🔍 [LINE-BALANCER] Word ${i} "${word.word}" check: current=${currentLineWidth.toFixed(1)}px, adding=${totalWidth.toFixed(1)}px, total=${(currentLineWidth + totalWidth).toFixed(1)}px, limit=${this.workingWidth.toFixed(1)}px, would exceed=${currentLineWidth + totalWidth > this.workingWidth}, has words=${currentLine.length > 0}`);

            // Check if adding this word would exceed our CSS 90% working width
    ...
    updateContainerWidth(container) {
        this.containerWidth = container.offsetWidth;
        this.workingWidth = Math.floor(this.containerWidth * 0.90);
    }
