...
class AudioControls {
    ...
    setupEventListeners() {
        ...
        // Play/Pause overlay button (new button)
        ...
        // (Reverted) Background play/pause click area removed
    ...
    async playAudio() {
        ...
        try {
            ...
            // Add playing class to play-pause overlay for opacity change
            const playPauseOverlay = document.getElementById('playPauseOverlay');
            if (playPauseOverlay) {
    ...
    pauseAudio() {
        if (this.audioSystem.currentAudioElement) {
            ...
            // Remove playing class from play-pause overlay
            const playPauseOverlay = document.getElementById('playPauseOverlay');
            if (playPauseOverlay) {
    ...
    togglePlayback() {
        if (!this.audioSystem.currentAudioElement) {
            console.warn('⚠️ [TOGGLE] No audio element available');
            return;
        }
        
        const isCurrentlyPaused = this.audioSystem.currentAudioElement.paused;
        
        console.log('▶️ [TOGGLE] togglePlayback called', {
            currentlyPaused: isCurrentlyPaused,
            overlayMode: document.getElementById('builtin-overlay')?.classList.contains('playback-mode')
        });
