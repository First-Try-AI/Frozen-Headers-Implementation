...
class SessionManager: ObservableObject {
    ...
    private func precalculateChunkHeights(for response: AudioResponse) {
        ...
        // CHANGED: Enforce minimum of 350pt
        let fixedHeight = max(screenHeight * 0.65, 350)
    ...
    private func finishSession() {
        print("LOG: [SessionManager] End of playback reached. Pausing for 2 seconds.")
        ...
        // 2. 2-Second Delay before returning to Reader Mode
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
            self.audioService.pause()
            self.audioService.seek(to: 0)
            
            // Soft transition (2.0s duration)
