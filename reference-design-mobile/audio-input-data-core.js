...
class AudioSystemInputDataCore {
    ...
    loadPagesData(pages) {
        ...
        console.log(`📄 [PAGE-DATA] Loaded ${pages.length} pages`);
        const pagesWithTransitions = pages.filter(p => p.transitionInfo).length;
        console.log(`🎯 [PAGE-DATA] ${pagesWithTransitions}/${pages.length} pages have transitionInfo`);

        if (pagesWithTransitions > 0) {
            console.log('🔄 [PAGE-DATA] Pages with transitionInfo:');
            pages.forEach((page, i) => {
                if (page.transitionInfo) {
                    console.log(`  Page ${i} (index ${page.pageIndex}): gap=[${page.transitionInfo.gapStartTime.toFixed(3)}s, ${page.transitionInfo.gapEndTime.toFixed(3)}s], midpoint=${page.transitionInfo.gapMidpointTime.toFixed(3)}s`);
                }
            });
        }
